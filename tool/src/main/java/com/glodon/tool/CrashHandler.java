...
//                mContext.startActivity(intent);
    ...
    private void saveFile(final String errorContent) {
        Runnable command=new Runnable() {
            @Override
            public void run() {
                String path = Environment.getExternalStorageDirectory().getPath() + "/com.glodon" +
                        "/log/";
                String fileName = DateUtils.getDate(DateUtils.YYYY_MM_DD_HH_MM) + ".log";
        ...
        for (Field field : fields) {
            try {
                ...
                Log.d("CHEN", "CrashHandler.NameNotFoundException---> an error occured when " +
                        "collect crash info: " + e.getMessage());
            }
        }
    }
